...
Output: {"skills":[{id,name,description,path}...]} sorted by name ascending.
...
async function warnOrphans(store: string, index: { contentFile: string }[]): Promise<void> {
  let files: string[];
  try {
  ...
  const known = new Set(index.map((e) => e.contentFile.replace(/^skills\//, "")));
  const orphans = files.filter((f) => f.endsWith(".md") && !known.has(f));
  if (orphans.length > 0) {
    warn(`orphaned content file(s) ignored by the library: ${orphans.join(", ")}`);
  }
}
